...
/***/ function(module, exports, __webpack_require__) {
	...
	window['MMIIndentedTree'] = component;
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * @fileOverview Visual indented tree for a strategic planning business model
	...
	(function () {
	    (function (factory) {
	        if (true) {
	            !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(3)], __WEBPACK_AMD_DEFINE_RESULT__ = function (d3) {
	                return factory(d3);
	            }.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	        } else {
	            window['MMIIndentedTree'] = factory(d3);
	        }
	         ...
	         * @param {IndentedTree} treeContext - Tree context
	         * @param {object} nodeContext - Data object from initial dataset related to the clicked node
	         */
	
	        /**
	         * Configuration options hash map passed to the indented tree on initialization
	         * @typedef {Object} IndentedTreeConfig
	         * @property {number} [width] - Width of the indented tree
	         * @property {number} [height] - Height of the indented tree
	         ...
	         * @property {array} [margin] Indented tree margin as array [top, right, bottom, left]
	         *
	         *
	         */
	         ...
	         * @property {IndentedTree} data.treeContext - Indented tree component context
	         * @property {object} data.nodeContext - Data related to the node event fired
	         * @property {string} data.action - Action performed
	         */
	
	        /**
	         * Visual indented tree for strategic planning business model. Uses d3 for visualization purposes and jQuery for working
	         * with document
	         * @param {HTMLElement} el
	         * @param {IndentedTreeConfig} config
	         * @fires {ActionEvent}
	         * @class
	         * @alias module:IndentedTree
	         * @constructor
	         */
	        var IndentedTree = function (el, config) {
	            /**
	             * HTML element indented tree is created in
	             * @type {HTMLElement}
	             */
	            this.el = el;
	            ...
	            /**
	             * Configuration object of the indented tree
	             * @type {IndentedTreeConfig}
	             */
	            this.config = {
	                width: 0,
	                height: 0,
	            ...
	            if (this.config.barWidth == 'auto') {
	                this.config.barWidth = 0.7 * this.config.width;
	            }
	        ...
	        /**
	         * Draws indented tree visualization using providing optional data. If data is not provided, use previous cached data
	         * If there is no previous data - throws Exception
	         * @param {object} data - Data to visualize
	         */
	        IndentedTree.prototype.draw = function (data) {
	            var width = this.config.width,
	                height = this.config.height;
	
	            var m = this.config.margin;
	
	            this.tree = d3.layout.tree().size([height - m[0] - m[2], width - m[1] - m[3]]).nodeSize([0, 20]);
	            this.vis = d3.select(this.el).append('svg')
	                .attr('class', 'indented-tree-vis')
	                .attr('width', '100%')
	                .attr('height', '800px')
	            ...
	            var SPAN_ID = '__ontherock_testspan';
	            this.testSpan = this.vis.append('text').style('visibility', 'hidden').attr('id', SPAN_ID);
	         ...
	         * Called in the context of indented tree object
	         * @private
	         */
	        IndentedTree.prototype._update = function (root) {
	            var _this = this,
	                config = this.config,
	                duration = config.duration;
	
	            // transition stash
	            var nodes = this.tree.nodes(this.dataset);
	
	            // Normalize for fixed-depth.
	            nodes.forEach(function (d, i) {
	                d.x = i * config.barHeight;
	            });
	            ...
	            var placeholder = nodeEnter.append('g')
	                .attr("transform", 'translate(0, ' + -config.barHeight / 2 + ')');
	
	            var back = placeholder.append('rect')
	                .attr('class', 'back')
	                .attr('height', config.barHeight);
	
	            preventTooltip(back);
	
	            nodeMenu(placeholder, config, this);
	            nodeButtonsAndBadges(placeholder, config, this.testSpan, this);
	        ...
	        /**
	         * Allows to connect to the events of indented tree. Just delegates its to root element events
	         * @param {string} eventType - Type of the event to connect to
	         * @param {Function} callback - Callback function
	         */
	        IndentedTree.prototype.on = function (eventType, callback) {
	         ...
	         * @private
	         */
	        IndentedTree.prototype._initContextMenu = function () {
	            var _this = this;
	            this.dropdown = document.body.appendChild(document.createElement('div'));
	            this.dropdown.className = 'indented-tree-dropdown';
	         ...
	         * @private
	         */
	        IndentedTree.prototype._openContextMenu = function (d) {
	            var html = '';
	         ...
	         * @private
	         */
	        IndentedTree.prototype._closeContextMenu = function () {
	        ...
	        /**
	         * Draws group with menu icon
	         * @param sel
	         */
	        function nodeMenu(sel, config, context) {
	            var g = sel.append('g').attr('class', 'menu');
	            ...
	            var top = (config.barHeight - h) / 2;
	
	            var right = config.barWidth - w - 5;
	        ...
	        /**
	         * Appends group with buttons
	         * @param sel
	         * @param d
	         * @param config
	         */
	        function nodeButtonsAndBadges(sel, config, testSpan, context) {
	            var nodes = sel.append('g').attr('transform', 'translate(0, 5)');
	
	            nodes.each(function (d) {
	                var buttons = viewinfo(d).buttons || [];
	
	                var node = d3.select(this);
	
	                var right = config.barWidth - 5 - 20 - 5;
	
	                buttons.forEach(function (button) {
	                    var name = button.name;
	
	                    var tbox = testSpan.text(name).node().getBBox();
	                    right -= (tbox.width + 10 + 10);
	
	                    node.append('rect')
	                        .attr("x", right)
	                        .attr("width", tbox.width + 10)
	                        .attr("y", 0)
	                        .attr("height", 30)
	                        .attr("fill", button.color || "#eee");
	
	                    node
	                        .append('text')
	                        .attr('class', 'button')
	                        .attr('x', right + 5)
	                        .attr('y', tbox.height / 2 + (config.barHeight - tbox.height) / 2)
	                        .text(name)
	                        .attr('fill', '#fff');
	                    ...
	                    preventTooltip(
	                        node.append('rect')
	                            ...
	                            .on('click', function (d) {
	                                ...
	                                emit(context.el, 'buttonclick', {
	                                    bubbles: true,
	                                    cancelable: true,
	                                    data: {treeContext: context, nodeContext: d, button: button}
	                                });
	
	                                if (config.buttonClick) {
	                                    config.buttonClick(button, context, d);
	                                }
	                            })
	                    )
	                    ;
	                ...
	                var badges = viewinfo(d).badges || [];
	
	                badges.forEach(function (badge) {
	                    var text = badge.text;
	
	                    var tbox = testSpan.text(text).node().getBBox();
	                    right -= (tbox.width + 10 + 10);
	                    ...
	                    node.append('rect')
	                        ...
	                        .on('mouseover', function (d) {
	                            d3.event.stopPropagation();
	                            tooltip(badge.hint, [d3.event.pageX + 3, d3.event.pageY + 3]);
	                        })
	                        .on('mousemove', function (d) {
	                            d3.event.stopPropagation();
	                            tooltip(badge.hint, [d3.event.pageX + 3, d3.event.pageY + 3]);
	        ...
	        /**
	         * Prevents showing tooltip on given nodes
	         * @param sel
	         */
	        function preventTooltip(sel) {
	        ...
	        function tooltip(text, coords) {
	            ...
	            var TOOLTIP_ID = tooltip.TOOLTIP_ID = '__mmi_indented_tree_tooltip';
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		if(!content.locals) {
			module.hot.accept("!!./../../../node_modules/css-loader/index.js!./../../../node_modules/less-loader/index.js!./IndentedTree.less", function() {
				var newContent = require("!!./../../../node_modules/css-loader/index.js!./../../../node_modules/less-loader/index.js!./IndentedTree.less");
...
/***/ function(module, exports, __webpack_require__) {
	...
	// module
	exports.push([module.id, ".indented-tree-vis .node {\n  cursor: pointer;\n  /*\n    this is important to handle mouse over and show tooltip. otherwise it will not be shown when\n    cursor is between icon and text\n    */\n  pointer-events: all;\n}\n.indented-tree-vis .link {\n  fill: none;\n  stroke: #aaa;\n  stroke-width: 1px;\n}\n.indented-tree-vis .menu-icon {\n  background-color: #333;\n  border-top: 4px solid #333;\n  border-bottom: 4px solid #333;\n  padding: 4px 0 4px 0;\n  height: 4px;\n}\n.indented-tree-dropdown {\n  position: absolute;\n  display: none;\n  border: 1px solid rgba(0, 0, 0, 0.2);\n  box-shadow: 0 0 5px -3px rgba(0, 0, 0, 0.4);\n  background: #fff;\n  z-index: 999;\n}\n.indented-tree-dropdown div {\n  height: 25px;\n  line-height: 25px;\n  padding: 0 15px 0 15px;\n  cursor: default;\n  color: #333;\n  border-bottom: 1px solid rgba(0, 0, 0, 0.1);\n}\n.indented-tree-dropdown div:last-child {\n  border-bottom: 0;\n}\n.indented-tree-dropdown div:hover {\n  background-color: rgba(0, 136, 204, 0.1);\n}\n.mmi-tooltip {\n  position: absolute;\n  padding: 10px;\n  border-radius: 5px;\n  background-color: rgba(0, 0, 0, 0.6);\n  color: #fff;\n}\n", ""]);
